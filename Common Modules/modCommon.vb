...
Module modCommon
    ...
    Private mObjectName As String = "" 'used by haseditsecurity and vieweditsecurity mrb 6/20/12
    ...
    Public Class SecurityMatrix
        ...
        Public EditRank As Integer = 0
        Public ViewRank As Integer = 0
    ...
    Public gUserList() As List(Of UserRecord)
    Public gSecurity() As SecurityMatrix
    Public gCADFileText As String
    Public gCADPathStd As String
    Public gsUserName As String
    'Public giAns As Integer
    Public Const JCPOHelpIndex = "JCPOHelp.chm"
    Public Const JCPOHelpFolder = "JCPOHelp" & "\"
    Public Const EstimatorHelpIndex = "EstimatorHelp.chm"
    Public Const EstimatorHelpFolder = "EstimatorHelp" & "\"
    Public gsJobExportPath As String
    Public gsExportServer As String
    ...
    Const iPackage = 0
    Const iDescription = 1
    Const iCONumber = 2
    Const iReviewedDate = 3
    Const iReviewedBy = 4
    Const iJobOptId = 5
    Const iCancelled = 6
    Const iReleased = 7
    ...
    Public Function IDERunning() As Boolean
        IDERunning = ProcessRunning("devenv") > 0
        Exit Function

ErrHandler:
        Debug.Print("Error in IDERunning: " & Err.Description)
        Debug.Assert(False)
    End Function
    ...
    Public Function ConnectDB()
        ...
        Dim sConnectionTest As String = ""
        Dim sConnectionQA As String = ""
        Dim sConnectionBOSS As String = ""
        Try
            'If IDERunning() Then
            ...
            If Not settings Is Nothing Then
                For Each cs As ConnectionStringSettings In settings
                    Select Case cs.Name
                        Case "HCHDataConnectionString"
                            sConnection = cs.ConnectionString
                        Case "HCHDataQAConnectionString"
                            sConnectionQA = cs.ConnectionString
                        Case "HCHDataTestConnectionString"
                            sConnectionTest = cs.ConnectionString
                        Case "HCHDataBOSSConnectionString"
            ...
            Select Case gsDatabase
                Case "HCHDataQA"
                    msConnection = sConnectionQA 'Global.BossAdmin.My.MySettings.Default.HCHDataQAConnectionString
                Case "HCHDataTest"
                    msConnection = sConnectionTest ' Global.BossAdmin.My.MySettings.Default.HCHDataConnectionString
                Case "HCHData"
                    msConnection = sConnection 'My.Settings.HCHDataConnectionString '5/13/15"Data Source=" & msdbServer & "; Initial Catalog = " & gsDatabase & ";User ID=boss; password=bosssa"
            End Select
            gsConnectionString = msConnection
            ...
            msConnectionBoss = sConnectionBOSS 'Global.BossAdmin.My.MySettings.Default.HCHDataBossConnectionString ' 5/13/15"Data Source=" & msdbServer & "; Initial Catalog = BOSS;User ID=boss; password=bosssa"

            gCN = New SqlConnection
            With gCN
                '.Provider = "SQLOLEDB"
            ...
            gCNBoss = New SqlConnection
            With gCNBoss
                '.Provider = "SQLOLEDB"
                .ConnectionString = msConnectionBoss
                '.CommandTimeout = 600
                .Open()
            End With
        ...
        Catch
            ConnectDB = False
            MsgBox("Database connection failed with error: " & Err.Description, True, "")
    ...
    '    End With
    'End Sub
    Public Sub CleanUpDS(pds As DataSet, Optional pcMD As SqlCommand = Nothing, Optional pad As SqlDataAdapter = Nothing)
        ...
        If Not IsNothing(pcMD) Then
            If Not pcMD Is Nothing Then pcMD = Nothing
            pcMD.Dispose()
        End If
    End Sub
    ...
    'End Function
    Public Function HasEditSecurity(sObjectName As String, sObjectType As String) As Boolean
        '6/21/12 mrb
        HasEditSecurity = False
        For Each SecurityMatrix In gSecurityList
            With SecurityMatrix
                If .ObjectName = sObjectName And .ObjectType = sObjectType And .EditRank <= giRank Then
                    HasEditSecurity = True
                    Exit For
                End If
            End With
        Next
    ...
    Public Function HasViewSecurity(sObjectName As String, sObjectType As String) As Boolean
        '6/21/12 mrb
        HasViewSecurity = False
        For Each SecurityMatrix In gSecurityList
            With SecurityMatrix
                If .ObjectName = sObjectName And .ObjectType = sObjectType Then
                    HasViewSecurity = True
                    Exit For
                End If
            End With
        Next
    ...
    Public Function SetMDICaption()
        Select Case gsDatabase
            ...
            Case "HCHDataQA"
                ...
                '==========================
                'gsVersion = "**QA DATA**  "
                gsVersion = "**COST ANALYSIS DATA**  "
        End Select    '}-> Select Case gsDatabase

        gsVersion = gsVersion & "Version: " & App.Major & "." & App.Minor & "." & App.Revision
        SetMDICaption = "B.O.S.S." & " " & gsVersion
    ...
    Private Function ProcessRunning(ByVal ProcessName As String) As Long
        Dim iReturn As Long
        Try
            iReturn = Convert.ToInt64(Process.GetProcessesByName(LCase(ProcessName)).GetUpperBound(0) + 1)
        Catch
            iReturn = 0
        End Try
        Return (iReturn)
    End Function
    Public Sub EnableDisableToolStrip(bEdit As Boolean, tsToolstrip As ToolStrip, grdGrid As UltraGrid, sObjectName As String, sObjectType As String)
        'mrb 7/27/12
        Dim bCanEdit As Boolean

        bCanEdit = HasEditSecurity(sObjectName, sObjectType) And Not bEdit
        ...
        If bEdit Then  'can only be bEdit if they were able to click Add or Edit button
    ...
    Public Sub InsertError(ByVal sStr As String, ByVal sErrorType As String, ByVal sCreatedBy As String)
        ...
        Try
            ...
            With oCMD
                ...
                .CommandText = "spErrorInsert"
                .Parameters.AddWithValue("@ErrorType", sErrorType)
                .Parameters.AddWithValue("@ErrorMessage", sStr)
                .Parameters.AddWithValue("@CreatedBy", sCreatedBy)
                .ExecuteNonQuery()
            End With

        Catch
            If Err.Number <> 0 Then
                MsgBox("Insert Error - UNHANDLED ERROR: " & Err.Number & ": " & Err.Description, vbCritical, "Insert Error")
            End If
        Finally
            oCMD = Nothing
        End Try
    ...
    Public Sub ShowError(sError As String, bSaveToDB As Boolean, sErrorType As String, sCalledby As String)
        MsgBox(sError, vbCritical, sCalledby)
        If bSaveToDB Then
            InsertError(sError, sErrorType, sCalledby & " " & gsUserID)
        End If
    End Sub
    Public Sub UltraGroupBoxReadOnly(ogBox As UltraGroupBox, bReadOnly As Boolean)
        Dim oControl As Control
        For Each oControl In ogBox.Controls
            Select Case True
                Case TypeOf oControl Is TextBox
                    DirectCast(oControl, TextBox).ReadOnly = bReadOnly
                Case TypeOf oControl Is UltraCombo
                    DirectCast(oControl, UltraCombo).ReadOnly = bReadOnly
                Case (TypeOf oControl Is UltraCheckEditor Or TypeOf oControl Is CheckBox)
    ...
    Public Function FileExists(ByVal sFileName As String) As Boolean
        ...
        Catch ex As Exception
            bExists = False
            MsgBox("Error check for File:" & sFileName & " Error: " & ex.Message, MsgBoxStyle.Critical, "FileExists")
        End Try
        Return bExists
    End Function
    Public Sub GetCostPlanPath()
        '11/13/14
        Dim rs As New DataSet
        Dim cDB As New DBCalls
        Dim sServer As String = Global.BossAdmin.My.MySettings.Default.FileServer

        If cDB.GetRecordsFromSP(rs, "spGetConfig", "PathCADCostNew") Then
            If rs.Tables(0).Rows.Count > 0 Then
                gsPathCost = sServer & rs.Tables(0).Rows(0).Item("ItemValue")
            End If
        End If
        'MsgBox("Path: " & gsPathCost, MsgBoxStyle.Information, "GetCostPlanPath")
    End Sub
    Public Sub ReloadCostAnalysisData()
        Dim cDB As DBCalls
        Dim i As Integer
        ...
        Try
            ...
            i = cDB.GetReturnValueFromSP("spCostAnalysisLookupsUpdate")
            MsgBox("Data has been refreshed.", vbInformation)
        Catch ex As Exception
            MsgBox("Error in Reload Cost Analysis Data: " & ex.Message, MsgBoxStyle.Critical, "ReloadCostAnalysisData")
        Finally
            mdiMain.UseWaitCursor = True
        End Try
    End Sub
    Public Sub ShowPDFHelp(sFileName As String)
        ...
        If bExists Then
            Dim oForm As New PDFViewer
            oForm.AxAcroPDF1.src = sFileName
            oForm.Show()
        Else
            MsgBox("The file you requested does not exist.  File: " & sFileName)
        End If
    End Sub
    Public Function SetGlobals() As Boolean
        Try
            gsUpdateFolder = My.Settings.UpdateFolder
        Catch ex As Exception
